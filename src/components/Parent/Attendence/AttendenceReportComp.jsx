...
const AttendenceReportComp = () => {
    ...
    // Children data from userData
    const children = useMemo(() => userData?.students || [], [userData]);

    // Selected child details
    const selectedChild = useMemo(() => 
        children.find(c => c.id === filters.studentId), 
    ...
    const getDisplayName = (type, id) => {
        if (type === 'student') return children.find(c => c.id === id)?.name || '';
            ...
            {isFilterExpanded && (
                <div className="p-6 space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <select
                                ...
                                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl outline-none text-sm font-medium disabled:opacity-50"
                            >
                                <option value="">{isLoadingSubjects ? 'Loading...' : 'Choose subject...'}</option>
                    ...
                    <div className="flex justify-end gap-4 pt-8 border-t border-gray-100">
                        ...
                        >
                            {attendanceSearch.isPending ? 'Fetching...' : <><Search size={18} /><span>View History</span></>}
                        </button>
                    </div>
                    ...
                    {studentReport && (
                        <div className="mt-12" ref={reportRef}>
                            <div className="flex justify-between items-center mb-8">
                                <h3 className="text-2xl font-black text-gray-900 tracking-tight">Child's Attendance</h3>
                                <button
                                    onClick={handlePrint}
                            ...
                            {studentReport.length > 0 ? (
                                ...
                                <div className="bg-amber-50 border border-amber-200 p-12 rounded-3xl text-center font-bold text-amber-800">No records found for this child in the selected criteria.</div>
                            )}
                        </div>
                    )}
                </div>
