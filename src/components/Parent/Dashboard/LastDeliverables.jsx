...
const LastDeliverables = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center flex-1 gap-3 py-2">
        <div className="relative flex items-center justify-center w-32 h-32">
          ...
          />
          <div
            className={`absolute inset-0 flex flex-col items-center justify-center ${isSidebarOpen ? "-z-50" : "z-auto"}`}
          >
            <span className="text-[9px] text-[#64748b] font-medium">Score</span>
            <span className="text-lg font-bold text-[#0B1053] leading-tight">
              {stats.percentage}%
            </span>
          </div>
        </div>
        <p className="text-xs font-semibold text-[#64748b] tracking-wide uppercase">Percentage</p>
      </div>
    );
  };
  ...
  const CustomGradeComponent = () => {
    return (
      <div className="flex flex-col items-center justify-center flex-1 gap-3 py-2">
        <div
          className="flex flex-col items-center justify-center w-32 h-32 rounded-full border-4 transition-all duration-300"
          style={{
            background: gc.bg,
            borderColor: gc.ring,
            boxShadow: `0 0 0 6px ${gc.bg}`,
          }}
        >
          <span className="text-4xl font-black" style={{ color: gc.text }}>
            {stats.grade}
          </span>
        ...
        <p className="text-xs font-semibold text-[#64748b] tracking-wide uppercase">Average Grade</p>
      </div>
    );
  };
      ...
      <div className="flex flex-col flex-1 gap-3">
        ...
        >
          ...
          <div className="flex items-center gap-2 pb-3 border-b border-[#f1f4ff]">
            ...
            <p className="text-sm font-semibold text-[#1a1a2e] truncate">{stats.title}</p>
          </div>
          ...
          <div className="flex flex-1 gap-4 items-center justify-center">
            ...
            {/* Divider */}
            <div className="w-px h-28 bg-[#f1f4ff]" />
            <div className="flex-1 flex flex-col items-center">
              <DeliverableComponent subjectQuery={subjectQuery} />
            </div>
          </div>
        </div>
      </div>
