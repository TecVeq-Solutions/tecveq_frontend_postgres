...
const RecentMessages = ({ onclose }) => {
  ...
  const { data: groupChats, isPending: groupIsPending } = useQuery({
    queryKey: ["parent-chatrooms"],
    queryFn: getMyChats
  });
  
  const { data: teachersResponse, isPending: teachersIsPending } = useQuery({
    queryKey: ["teachers-for-parent-chat"],
    queryFn: getTeachersForChat
  });
  ...
  const combinedUsers = [...(adminsResponse || []), ...(teachersResponse?.data || teachersResponse || [])];
  ...
  const groupedUsers = filteredUsers.reduce((acc, user) => {
    ...
    if (!acc[role].some(existing => existing.id === user.id)) {
        acc[role].push(user);
    }
    return acc;
  }, {});
  ...
  const groupsList = groupChats?.data || groupChats || [];
  const groupedGroups = groupsList.reduce((acc, chat) => {
    // Only group chats usually have classrooms or we can just categorize by classname
    ...
    <div ref={containerRef} className="fixed right-0 inset-y-0 z-[2000] flex flex-row-reverse items-start pointer-events-none h-full w-full sm:w-auto overflow-hidden">
      ...
      <div onMouseDown={(e) => e.stopPropagation()} className={`${showFullChat ? "hidden sm:flex" : "flex"} flex-col bg-white sm:w-96 w-full shadow-2xl border-l border-gray-100 pointer-events-auto h-full animate-chat-panel`} style={{ boxShadow: "-10px 0 30px -15px rgba(0,0,0,0.1)" }}>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-gray-50/30">
          {teachersIsPending || groupIsPending ? <div className="flex justify-center p-10"><Loader /></div> : (
            <>
